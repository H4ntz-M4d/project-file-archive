...
var KTKategoriList = (function () {
    var t, e;

    const handleDeleteButtons = () => {
        e.querySelectorAll('[data-kt-kategori-table-filter="delete_row"]').forEach((el) => {
            el.addEventListener("click", function (ev) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: `/kategori-surat/destroy/${row.dataset.slug}`, // Pastikan `data-slug` ada di <tr>
                            type: 'DELETE',
                            data: {
    ...
    const handleGroupActions = () => {
        ...
        const deleteSelectedBtn = document.querySelector('[data-kt-kategori-table-select="delete_selected"]');
        ...
        deleteSelectedBtn.addEventListener("click", function () {
            ...
            Swal.fire({
                text: "Yakin ingin menghapus pilihan kategori?",
                icon: "warning",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonText: "Yes, delete!",
                ...
                if (result.value) {
                    $.ajax({
                        url: '/kategori-surat/delete-selected',
                        type: 'DELETE',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content'),
                        ...
                        success: function (res) {
                            if (res.success) {
                                Swal.fire({
                                    text: "Kamu berhasil menghapus pilihan kategori!",
                                    icon: "success",
                                    buttonsStyling: false,
                                    confirmButtonText: "Ok, got it!",
                                    customClass: { confirmButton: "btn fw-bold btn-primary" },
    ...
    const updateToolbar = () => {
        const baseToolbar = document.querySelector('[data-kt-kategori-table-toolbar="base"]'),
            selectedToolbar = document.querySelector('[data-kt-kategori-table-toolbar="selected"]'),
    ...
    return {
        init: function () {
            e = document.querySelector("#kt_kategori_table");
            if (!e) return;

            t = $(e).DataTable({
                processing: true,
                serverSide: true,
                ajax: "/kategori-surat/data-list", // ganti sesuai route kamu
                columns: [
            ...
            // Search input
            document.querySelector('[data-kt-kategori-table-filter="search"]')
                .addEventListener("keyup", function (event) {
                    t.search(event.target.value).draw();
                });

            // Optional: Dropdown filter status
            const filter = document.querySelector('[data-kt-kategori-table-filter="status"]');
            if (filter) {
                $(filter).on("change", (e) => {
